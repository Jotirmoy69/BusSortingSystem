...
export default function DayShift() {
  ...
  const handleSave = () => {
    ...
    const tabStands = tempSelect[activeTab] || [];
    if (tabStands.length === 0) {
      toast.error(`Please select at least one stand for ${activeTab}!`);
      setSaving(false);
      return;
    }

    const allStands = tabStands.map((stand) => ({
      name: normalizeStandName(stand.name),
      total: Number(stand.students),
      gender: stand.gender || activeTab,
    }));

    const totalStudents = allStands.reduce(
      (sum, s) => sum + (s.total || 0),
      0
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                >
                  <motion.select
                    ...
                    onChange={(e) => {
                      const bus = (activeBuses || []).find(
                        (b) =>
                          String(b.number) === e.target.value ||
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-xs font-medium px-2 py-1 bg-purple-100 text-purple-800 rounded-full">
                {(tempSelect.boys || []).length +
                  (tempSelect.girls || []).length}{" "}
                selected
              </span>
            </div>
            ...
            <div className="flex border-b border-gray-200 mb-4">
              <button
                ...
                onClick={() => setActiveTab("boys")}
              >
                Boys {(tempSelect.boys || []).length > 0 ? `(${(tempSelect.boys || []).length})` : ""}
              </button>
              <button
                className={`py-2 px-4 font-medium text-sm flex items-center gap-2 ${
                  activeTab === "girls"
                ...
                onClick={() => setActiveTab("girls")}
              >
                Girls {(tempSelect.girls || []).length > 0 ? `(${(tempSelect.girls || []).length})` : ""}
              </button>
            </div>
                      ...
                      {filteredStands.map((stand, sidx) => (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-semibold bg-purple-600 text-white px-2 py-0.5 rounded">
                              {activeTab === "boys" ? stand.boys : stand.girls} {activeTab}
                            </span>
                          </div>
