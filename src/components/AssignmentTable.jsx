...
//   const isNew = location.pathname === '/new'; // ✅ /new flag
//   const showActions = ['/morning', '/day','/college'].includes(location.pathname); // ✅ Only show on these paths
//   const showStats = !isNew;
//    const [fontSize, setFontSize] = useState("text-base"); // ✅ Hide Capacity, Assigned, Utilization on /new
...
//   // ✅ Accurate column count for empty state row
//   const columnCount =
//     1 + // Bus ID
//     (showStats ? 3 : 0) + // Capacity, Assigned, Utilization
//     (showGender ? 1 : 0) + // Gender
//     1 + // Stands
...
//         <motion.table
//           className={`w-full shadow-lg rounded-md overflow-hidden ${isNew ? 'text-lg md:text-xl' : ''}`} // ✅ Bigger font on /new
//           initial="hidden"
//           animate="visible"
  ...
  return (
    <div className="mt-2">
      ...
      >
                ...
                {assignedBuses.map((bus) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {bus.number || bus.id || "N/A"} ({bus.gender || "N/A"})
                      </td>
                      ...
                      <td className={`border border-gray-300 p-3 cursor-pointer`}>
                        {bus.stands && bus.stands.length > 0 ? (
                          <motion.ul
                            className="flex flex-wrap gap-2"
                            initial="hidden"
        ...
        {location.pathname === "/new" && (
        <span className="fixed bottom-1 left-2 text-gray-500">Developed By Jotirmoy || Designed By Raiyan || Copyright © 2025</span>
        )}
    </div>
  );
};
